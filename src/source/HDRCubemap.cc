...
HDRCubemap::HDRCubemap(const string& path, s_ptr<Shader> BackgroundShade, s_ptr<Shader> EquirectangularToCubemapShader, s_ptr<Shader> IrrandanceShader, s_ptr<Shader> PrefilterShader, s_ptr<Shader> BRDFShader)
{
	path_ = path;
...
void HDRCubemap::LoadHDRimg(GLFWwindow* window, s_ptr<llr::Camera> camera)
{
    ...
    float* data = stbi_loadf(path_.c_str(), &width, &height, &nrComponents, 0);
    ...
    EquirectangularToCubemapShader_->SetMatrix4("projection_matrix", captureProjection);
    ...
    for (unsigned int i = 0; i < 6; ++i)
    {
        EquirectangularToCubemapShader_->SetMatrix4("view_matrix", captureViews[i]);
   ...
   // --------------------------------------------------------------------------------
    ...
    for (unsigned int i = 0; i < 6; ++i)
    {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, irradanceMap, 0);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
   ...
   // --------------------------------------------------------------------------------
    ...
    PrefilterShader_->SetMatrix4("projection_matrix", captureProjection);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap);
    ...
    for (unsigned int mip = 0; mip < maxMipLevels; ++mip)
    {
        ...
        for (unsigned int i = 0; i < 6; ++i)
        {
            PrefilterShader_->SetMatrix4("view_matrix", captureViews[i]);
