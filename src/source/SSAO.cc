...
void SSAOBuffer::Init(int height, int width)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, float(width) * 0.5f, float(height) * 0.5f, 0, GL_RGBA, GL_FLOAT, NULL);
...
void SSAOBlurBuffer::Init(int height, int width)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, float(width) * 0.5f, float(height) * 0.5f, 0, GL_RGBA, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
