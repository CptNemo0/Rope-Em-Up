...
void GBuffer::BindTextures()
{
	...
	glBindTexture(GL_TEXTURE_2D, position_texture_);
	light_pass_shader->SetInt("position_texture", position_texture_);

	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, normal_texture_);
	light_pass_shader->SetInt("normal_texture", normal_texture_);

	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, albedo_texture_);
	light_pass_shader->SetInt("albedo_texture", albedo_texture_);

	glActiveTexture(GL_TEXTURE3);
	glBindTexture(GL_TEXTURE_2D, mra_texture_);
	light_pass_shader->SetInt("mra_texture", mra_texture_);
}